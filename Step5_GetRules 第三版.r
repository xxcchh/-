# 程序说明
# 当前版本： step5生成规则V6.0 第三版



#-----------------------------------------------------------------------------------------------
# 版本号：step5生成规则V6.0 第三版
# 更新时间：2016.8.3
# 作者：Crystal
# 更新内容：
...
# 作者：Crystal
# 更新内容：
# 1）修改了Interest，rulesmining，seqcovermining，R6Class（两个类都改了）函数，在step9产生全局最优规则的基础上把前项限制为一定包括appCols
# 2）修改了rnode$GetFrameNew，解决当n=12出现的bug。（原因是计算出的interest=0的规则在step9导致的错误）
#-----------------------------------------------------------------------------------------------
...
Interest <- function(supp, conf, lambda, mu){
  # lambda为合格率
  beta <- function(num = 0.5){
    val <- 1 + lambda^num
    return (val)
  }
  # val <- ((conf - lambda) / lambda) * ((supp * beta() - lambda*mu) / (lambda*mu))
  val <- ((conf - lambda) / lambda) * ((supp - lambda*mu) / (lambda*mu))
  # val <- ((conf - lambda) / lambda) * ((supp/lambda - lambda*mu) / (lambda*mu))
  return(val)
}
