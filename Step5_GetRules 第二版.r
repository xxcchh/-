# 程序说明
# 当前版本： step5生成规则V6.0 第二版


#-----------------------------------------------------------------------------------------------
# 版本号：step5生成规则V6.0 第二版
# 更新时间：2016.8.3
# 作者：Crystal
# 更新内容：
# 1）修改了rnode$GetRawName函数，解决当discdata中数据不为数字时候的规则表示问题。
#-----------------------------------------------------------------------------------------------
#-----------------------------------------------------------------------------------------------
# 版本号：step5生成规则V6.0 第二版
# 更新时间：2016.8.2
# 作者：Crystal
# 更新内容：
# 1）修改了Interest（订正公式）, R6Class（两个类都改了）修正了bug（主要原因是规则中的空格引起的）。
#-----------------------------------------------------------------------------------------------
...
# 作者：Crystal
# 更新内容：
# 1）修改了Interest，rulesmining，seqcovermining，R6Class（两个类都改了）函数，改变step9产生规则的方式，找到不同个数前项的全局最优规则。
#-----------------------------------------------------------------------------------------------

#-----------------------------------------------------------------------------------------------
# 版本号：step5生成规则V6.0 第一版
# 更新时间：2016.7.29
# 作者：Crystal
# 更新内容：
# 1）修改了Interest，rulesmining，seqcovermining，R6Class（两个类都改了）函数，改变了Interest的定义 
#-----------------------------------------------------------------------------------------------
...
# 作者：Crystal
# 更新内容：
# 1）修改R6Class(RulesTree), 解决result$rules单个规则重复显示问题。
#-----------------------------------------------------------------------------------------------
...
# 作者：Crystal
# 更新内容：
# 1）修改R6Class，修改puritylossAndCov呈现的bug。
# 2）修改了GLFX，之前没有把appCols作为参数输入。
#-----------------------------------------------------------------------------------------------
...
# 作者：Crystal
# 更新内容：
# 1）修改R6Class，解决bug1-2（前项只有一个的规则开始删减前项产生的bug）。
# 2）修改了GLFX，rulesmining，解决bug3（当某时刻规则数为0的时候提前结束，输出之前步骤得到的规则）。
#-----------------------------------------------------------------------------------------------

#-----------------------------------------------------------------------------------------------
# 时间：2016年7月25日20:50:50
# 目的：step5生成规则V2.0
#-----------------------------------------------------------------------------------------------
...
#useNA表示是否支持空值，如果不支持就会报错,默认为支持，此接口需开放给后台
##########
#输出参数接口说明：
...
rulesmining <- function(data_pre_handle, GainsR_value, appCols, conCols, n=10, mu=0.2, eta=0.5,
  ...
  res <- seqcovermining(datatemp, rules, rate, maxLhs, mu, theta)
  rules <- res$rules
  ruleslist[[8]] <- rules
...
# seqcovermining -------------------------------------------------------------
seqcovermining <- function(data, rules, rate, maxLhs, mu, theta){
  rulestree <- RulesTree$new(data, rules, rate, maxLhs, mu, theta)
  rules <- rulestree$rules
  prules <- rulestree$prules
  df <- rulestree$pdf
  return(list(rules = rules, prules = prules, pdf = df))
}
...
rnode <- R6Class("rnode", #节点
                 ...
                 public = list(
                   ...
                   # 删减前项最优规则
                   DeleteItemNew = function(N, lambda, mu){
                     children <- self$GetAllChildren()
                   ...
                   # 在删减前项的过程中寻找全局最优的组合
                   GetFrameNew = function(children, m, n, N, lambda, mu){
                     item <- NULL
                     maxint <- 0
                     suppi <- 0
                     confi <- 0
                     ...
                     for (k in numlist){
                      ...
                      for (i in 1:m){
                        ...
                        name <- c$GetRawName()
                        df[i, 'name'] <- name
                        df[i, 'purityloss'] <- ifelse(m > 1, confi - conf, confi)
                        df[i, 'coverage'] <- length(c$supp) / N
                      }
                      item <- unlist(sapply(children[item], function(x) x$name))
                      res <- list(supp = suppi, conf = confi, interest = maxint)
                      return(list(item = item, info = res, df = df))
                  }
                 ),
...
RulesTree  <- R6Class('RulesTree', #产生规则树
                      ...
                      public = list(
                        ...
                        initialize = function(data, rules, prep, maxLhs, mu, theta){
                        ...
                        GetHierarchyRules = function(){
                          ...
                          for (i in 1:n){
                            tri <- tr[[i]]
                            dfi <- data.frame()
                            res <- tri$DeleteItemNew(private$N, private$prep, private$mu)
